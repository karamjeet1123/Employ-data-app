...
def f1_1():
	...
	try:
		#STORAGE
		con=MongoClient(con=MongoClient("mongodb://localhost:27017/")
)
		db=con["ssnov22"]
		coll=db["employee"]
		...
		try:
			...
			showerror("issue","name should not be non alphabetical/empty/less than 2 aplphabates")
			return

		try:
			salary=float(aw_ent_salary.get())
			if salary<8000.0 or len(str(salary))==0:
				raise Exception("salary should be less than 8000 and not empty")
		except Exception:
...
def f2():
	...
	try:
		#STORAGE
		con=MongoClient("mongodb+srv://karamjeetkaur3399:12345@cluster0.ew0zfwv.mongodb.net/")
		db=con["ssnov22"]
		coll=db["employee"]
...
#main window
mw=Tk()
#mw.iconbitmap(r"C:\Users\welcome\Downloads\Employee-Data-App-main\Employee-Data-App-main\Employee-Data-App\Employees.ico")

bg_color="LightPink1"
btn_color="light steel blue"
mw.configure(bg=bg_color)
mw.title("Employees Management System")
mw.geometry("800x800+50+50")
f=("Arial",30,"bold")

mw_btn_add=Button(mw,text="Add",font=f,width=10,command=f1,bg=btn_color)
mw_btn_view=Button(mw,text="View",font=f,width=10,command=f2,bg=btn_color)
mw_btn_update=Button(mw,text="Update",font=f,width=10,command=f3,bg=btn_color)
mw_btn_delete=Button(mw,text="Delete",font=f,width=10,command=f4,bg=btn_color)
mw_btn_chart=Button(mw,text="charts",font=f,width=10,command=f5,bg=btn_color)
...
mw_lab_loc=Label(mw,text=loctn,font=f,bg=bg_color)

mw_lab_temp=Label(mw,text=tempt,font=f,bg=bg_color)
...
bg_color="LightSkyBlue3"
btn_color="misty rose"
aw.configure(bg=bg_color)
aw.title("Add employee data")
aw.geometry("800x800+50+50")

aw_lab_eno=Label(aw,text="enter employee ID ",font=f,bg=bg_color)
aw_ent_eno=Entry(aw,font=f)

aw_lab_name=Label(aw,text="enter name",font=f,bg=bg_color)
aw_ent_name=Entry(aw,font=f)

aw_lab_salary=Label(aw,text="enter salary",font=f,bg=bg_color)
aw_ent_salary=Entry(aw,font=f)

aw_btn_add=Button(aw,text="Add",font=f,width=10,command=f1_1,bg=btn_color)
aw_btn_back=Button(aw,text="Back",font=f,width=10,command=f1_2,bg=btn_color)
...
bg_color="DarkSeaGreen1"
btn_color="LavenderBlush4"
...
vw_btn_back=Button(vw,text="Back",font=f,command=f2_1,bg=btn_color)
...
bg_color="plum1"
btn_color="thistle"
uw.configure(bg=bg_color)
uw.title("Update employee data")
uw.geometry("800x800+50+50")

uw_lab_eno=Label(uw,text="enter employee ID ",font=f,bg=bg_color)
uw_ent_eno=Entry(uw,font=f)

uw_lab_name=Label(uw,text="enter name",font=f,bg=bg_color)
uw_ent_name=Entry(uw,font=f)

uw_lab_salary=Label(uw,text="enter salary",font=f,bg=bg_color)
uw_ent_salary=Entry(uw,font=f)

uw_btn_update=Button(uw,text="update",font=f,width=10,command=f3_1,bg=btn_color)
uw_btn_back=Button(uw,text="Back",font=f,width=10,command=f3_2,bg=btn_color)
...
bg_color="LavenderBlush3"
btn_color="firebrick1"
dw.configure(bg=bg_color)
dw.title("Delete employee data")
dw.geometry("800x800+50+50")

dw_lab_eno=Label(dw,text="enter employee ID ",font=f,bg=bg_color)
dw_ent_eno=Entry(dw,font=f)

dw_lab_eno.pack(pady=10)
dw_ent_eno.pack(pady=10)

dw_btn_delete=Button(dw,text="Delete",font=f,width=10,command=f4_1,bg=btn_color)
dw_btn_back=Button(dw,text="Back",font=f,width=10,command=f4_2,bg=btn_color)
